...
var _sound_stream_players: Dictionary[int, SoundStreamPlayer]
var _sound_2d_stream_players: Dictionary[int, SoundStreamPlayer2D]
var _sound_3d_stream_players: Dictionary[int, SoundStreamPlayer3D]
var _music_stream_players: Dictionary[int, MusicStreamPlayer]

# Available (idle) players pools
var _available_sound_stream_players: Array[SoundStreamPlayer]
var _available_sound_2d_stream_players: Array[SoundStreamPlayer2D]
var _available_sound_3d_stream_players: Array[SoundStreamPlayer3D]
var _available_music_stream_players: Array[MusicStreamPlayer]
...
## Play a sound from an Aud3ioStream resource.
...
func _find_existing_non_positional_player(stream: AudioStream) -> SoundStreamPlayer:
...
func _find_existing_2d_player_on_parent(parent: Node, stream: AudioStream) -> SoundStreamPlayer2D:
...
func _find_existing_3d_player_on_parent(parent: Node, stream: AudioStream) -> SoundStreamPlayer3D:
...
func play_loaded_music(stream_name: String, position: float = 0.0,
		...
		override_bus: String = "", override_process_mode: int = PROCESS_MODE_NO_OVERRIDE) -> MusicStreamPlayer:
	var stream: AudioStream = null
...
## Play music from an AudioStream resource.
func play_music(stream: AudioStream, position: float = 0.0,
	...
	var stream_player: MusicStreamPlayer = null
	...
	if stream_player:
		...
		if volume_transition_in_duration > 0.0:
			...
			stream_player.create_tween_volume_transition_in(volume_linear, volume_transition_in_duration)
		else:
			stream_player.volume_db = linear_to_db(volume_linear)
...
func stop_music(stream_player: MusicStreamPlayer,
	...
	if volume_transition_out_duration > 0.0:
		var tween := stream_player.create_tween_volume_transition_out(volume_transition_out_duration)
...
func is_playing(stream_player: Node) -> bool:
	...
	if stream_player is SoundStreamPlayer \
			or stream_player is SoundStreamPlayer2D \
			or stream_player is SoundStreamPlayer3D \
			or stream_player is MusicStreamPlayer:
...
func is_music_playing(stream_name: String) -> bool:
	...
	for player in _music_stream_players.values():
		if is_instance_valid(player) and player.playing and player.stream == target_stream:
			return true
...
func pause_sound(stream_player: Node) -> bool:
	...
	if stream_player is SoundStreamPlayer \
			...
			or stream_player is SoundStreamPlayer3D \
			or stream_player is MusicStreamPlayer:
		if stream_player.playing and not stream_player.is_paused():
			stream_player.pause_playback()
			return true
	else:
		push_error("Invalid stream player type: ", stream_player.get_class())
...
func resume_sound(stream_player: Node) -> bool:
	...
	if stream_player is SoundStreamPlayer \
			...
			or stream_player is SoundStreamPlayer3D \
			or stream_player is MusicStreamPlayer:
		if stream_player.is_paused():
			stream_player.resume_playback()
			return true
	else:
		push_error("Invalid stream player type: ", stream_player.get_class())
...
func is_paused(stream_player: Node) -> bool:
	...
	if stream_player is SoundStreamPlayer \
			...
			or stream_player is SoundStreamPlayer3D \
			or stream_player is MusicStreamPlayer:
		return stream_player.is_paused()
...
func pause_all_sounds_by_type(sound_type: SoundType) -> void:
	...
	for player in players.values():
		if is_instance_valid(player) and player.playing and not player.is_paused():
...
func resume_all_sounds_by_type(sound_type: SoundType) -> void:
	var players := _get_players_by_sound_type(sound_type)
	for player in players.values():
		if is_instance_valid(player) and player.is_paused():
...
func pause_music(stream_player: MusicStreamPlayer) -> bool:
	if not is_instance_valid(stream_player):
		return false
...
func resume_music(stream_player: MusicStreamPlayer) -> bool:
	if not is_instance_valid(stream_player):
		return false
...
func toggle_music_pause(stream_player: MusicStreamPlayer) -> bool:
	if not is_instance_valid(stream_player):
		return false

	if stream_player.playing and not stream_player.is_paused():
		return pause_music(stream_player)
	elif stream_player.is_paused():
...
func _update_sound_2d_channels(count: int) -> void:
	...
	if count > current_count:
		for i in count - current_count:
			var sp := SoundStreamPlayer2D.new()
			sp.bus = SOUND_BUS_NAME
...
func _update_sound_3d_channels(count: int) -> void:
	...
	if count > current_count:
		for i in count - current_count:
			var sp := SoundStreamPlayer3D.new()
			sp.bus = SOUND_BUS_NAME
...
func _remove_connection_tree_exiting(stream_player: Node) -> bool:
	...
	if stream_player is SoundStreamPlayer2D:
...
## Finds the oldest player that meets the priority criteria for sounds
...
func _on_sound_stream_player_finished(stream_player: SoundStreamPlayer) -> void:
	if stream_player:
...
func _on_sound_2d_stream_player_finished(stream_player: SoundStreamPlayer2D) -> void:
	if stream_player:
		var stream_name := _get_stream_name(stream_player.stream)
...
func _on_sound_3d_stream_player_finished(stream_player: SoundStreamPlayer3D) -> void:
	if stream_player:
		var stream_name := _get_stream_name(stream_player.stream)
...
func _on_music_stream_player_finished(stream_player: MusicStreamPlayer) -> void:
	if stream_player:
		var stream_name := _get_stream_name(stream_player.stream)
